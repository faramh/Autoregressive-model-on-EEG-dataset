function [model, R2, whiteness, Y_hat,Y_test_plus_hat, runtime, R2_lagremoved,whiteness_lagremoved] = linear_AR(Y, include_W, n_AR_lags, W_mask, k, use_parallel, test_range)
...
% Exclude the initial n_AR_lags points from the accuracy calculations
Y_test_plus_hat_valid = Y_test_plus_hat(:, n_add_AR_lags:end);
Y_test_plus_valid = Y_test_plus(:, n_add_AR_lags:end);



% Prediction error for valid points
E_test_lagremoved = Y_test_plus_hat_valid - Y_test_plus_valid;

% Compute R2 for valid points
R2_lagremoved = 1 - sum(E_test_lagremoved.^2, 2) ./ sum((Y_test_plus_valid - mean(Y_test_plus_valid, 2)).^2, 2);

% Perform the whiteness test on valid points
[whiteness_lagremoved.p1, whiteness_lagremoved.stat1, whiteness_lagremoved.sig_thr] = my_whitetest_multivar(E_test_lagremoved);
